...
data "local_file" "missing_cluster_check" {
  ...
  lifecycle {
    precondition {
      condition     = length(local.prism_element_missing_clusters) == 0
      error_message = "ERROR: The following configured clusters were not found in Nutanix - deployment has been aborted: ${jsonencode(local.prism_element_missing_clusters)}"
    }
  }
}
